...
-- يحل مشكلة النتائج الناقصة عند الفترة الواسعة (2000-01-01)
...
BEGIN
  ...
  -- سجلات من الداشبورد: من activity_logs
  recs_dashboard AS (
  ...
  -- آخر نشاط مطلق (خارج النطاق) للعرض
  last_act_global AS (
  ...
  -- عدد الحركات للمستخدم في النطاق (لـ total_actions)
  user_act_count AS (
    SELECT al.user_id AS uid, COUNT(*)::bigint AS cnt
    FROM public.activity_logs al
    WHERE al.user_id IS NOT NULL
