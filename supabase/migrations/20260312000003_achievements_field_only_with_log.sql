-- سجلات ميدانية = فقط عندما يوجد دليل أن المحصل أنشأ/رفع السجل من التطبيق (activity_log create_record بدون from_dashboard)
-- السجلات المستوردة أو المربوطة من الداشبورد بدون هذا السجل لا تُحسب كميدانية
...
BEGIN
  ...
  -- سجلات ميدانية: فقط سجلات أنشأها/رفعها المحصل من التطبيق (يوجد سجل نشاط create_record بدون from_dashboard من نفس المستخدم)
  recs_submitted AS (
    ...
    WHERE cr.field_agent_id IS NOT NULL
      ...
      AND EXISTS (
        ...
        WHERE al.target_id = cr.id
          ...
          AND al.user_id = cr.field_agent_id
      )
...
COMMENT ON FUNCTION public.get_users_achievements(text, text) IS 'سجلات ميدانية = فقط سجلات فيها create_record من التطبيق (بدون from_dashboard) من نفس المحصل';
