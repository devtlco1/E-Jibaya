-- تدقيق = عدد السجلات المدققة (كل سجل يُحسب مرة واحدة) وليس عدد أحداث verify_record
-- استخدام COUNT(DISTINCT target_id) لتجنب تكرار العد عند تسجيل نفس السجل مرتين
...
COMMENT ON FUNCTION public.get_users_achievements(text, text) IS 'إنجازات المستخدمين — عمود تدقيق = عدد السجلات المميزة المدققة (COUNT DISTINCT target_id)';
