...
export const dbOperations = {
  ...
  // قفل مقارنة الصور
  async lockPhotoViewing(recordId: string, userId: string): Promise<boolean> {
    try {
      const client = checkSupabaseConnection();
      if (!client) {
        throw new Error('فشل في الاتصال بقاعدة البيانات');
      ...
      // التحقق من أن السجل غير مقفل لمقارنة الصور من قبل مستخدم آخر
      const { data: existingRecord, error: fetchError } = await client
        .from('collection_records')
        .select('photo_viewing_by, photo_viewing_at')
        .eq('id', recordId)
        .single();

      if (fetchError) {
        console.error('Error fetching record for photo viewing lock:', fetchError);
        return false;
      }

      if (existingRecord.photo_viewing_by && existingRecord.photo_viewing_by !== userId) {
        console.log('Record is already being viewed by another user');
        return false;
      }
      ...
      const { error } = await client
        .from('collection_records')
        .update({
          photo_viewing_by: userId,
          photo_viewing_at: new Date().toISOString()
        })
        .eq('id', recordId);

      if (error) {
        console.error('Lock photo viewing error:', error);
        return false;
      }
  ...
  // إلغاء قفل مقارنة الصور
  async unlockPhotoViewing(recordId: string, userId: string): Promise<boolean> {
    try {
      const client = checkSupabaseConnection();
      if (!client) {
        throw new Error('فشل في الاتصال بقاعدة البيانات');
      ...
      const { error } = await client
        ...
        .eq('id', recordId)
        .eq('photo_viewing_by', userId);

      if (error) {
        console.error('Unlock photo viewing error:', error);
        return false;
      }
